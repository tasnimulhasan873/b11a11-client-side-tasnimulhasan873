...
const FoodDetails = () => {
    ...
    const fetchFood = async () => {
      try {
        const res = await axios.get(
          `https://a11-food-sharing-server-nine.vercel.app/food/${id}`
        );
        setFood(res.data);
      } catch (err) {
  ...
  const handleRequest = async () => {
    ...
    try {
      ...
      await axios.patch(
        `https://a11-food-sharing-server-nine.vercel.app/food/${food._id}`,
        { status: "requested" }
      );

      // 2. Add to requested foods
      await axios.post(
        `https://a11-food-sharing-server-nine.vercel.app/requested-foods`,
        requestData
      );
